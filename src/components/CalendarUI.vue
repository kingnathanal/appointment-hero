      ...
      <div class="card rounded-0 shadow-sm">
        <div class="card-header bg-light">
          <div class="row">
            ...
            <div class="col">
              <span class="float-right" @click="next()">
...
export default {
  ...
  methods: {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
          if (i === 0 && j < firstDay) {
            ...
            let card;
            if (
              date === this.currValues.date &&
              month === this.currValues.month &&
              year === this.currValues.year
            ) {
              card = this.makeCard(
                date,
                "card card-cal-date text-left bg-info text-white p-1 rounded-0"
              );
            } else {
              card = this.makeCard(
                date,
                "card card-cal-date text-left p-1 rounded-0 calday"
              );
            }
            row.appendChild(card);
            date++;
    ...
    previous() {
      let year =
        this.selectedValues.month === 0
          ? this.selectedValues.year - 1
          : this.selectedValues.year;
      let month =
        this.selectedValues.month === 0 ? 11 : this.selectedValues.month - 1;
      this.selectedValues = { month: month, year: year };
      this.createCalendar(this.selectedValues.month, this.selectedValues.year);
    },
    jump() {
  ...
  mounted() {
    this.selectedValues = {
      month: this.selectedFromApp.month,
      year: this.selectedFromApp.year
    };
  },
  updated() {},
  computed: {
...
//document.querySelector(".calday").onclick
//document.querySelector(".calday").addEventListener("click", function(e) {
